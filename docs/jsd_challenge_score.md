...
- `jsd_vs_L0` (float): Jensen–Shannon distance in base 2 between byte histograms (0..1).
...
- `w_jsd=0.45`, `w_dH=0.20`, `w_dD=0.20`, `w_changed=0.10`, `w_dsize=0.05`.
...
## Related (images-only variant)
A separate image-based challenge score exists in `process-check/scripts/images/evaluate_image_obfuscation_challenge.py`:
- `challenge_score = JSD + (1 - SSIM) + w_ent * entropy_diff` (used for image validations; distinct from the binary metric above).

## Defaults and CLI flags
These defaults come from `tools/binary_diversity.py`:
- Weights: `--w-jsd 0.45 --w-dH 0.20 --w-dD 0.20 --w-changed 0.10 --w-dsize 0.05`.
- L2 gates: `--l2-min-changed 0.008 --tau-l2 0.10`.
